...
class RayPrimitive extends DrawingPrimitiveBase {
  ...
  _draw({
    ...
    if (Math.abs(dx) > 1e-6) {
      ...
      ex = x1;
      ey = dy >= 0 ? bitmapSize.height / verticalPixelRatio : 0;
    }
    ctx.save();
    ctx.strokeStyle = this.color;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(x1 * horizontalPixelRatio, y1 * verticalPixelRatio);
    ctx.lineTo(ex * horizontalPixelRatio, ey * verticalPixelRatio);
    ctx.stroke();
    ctx.restore();
  }
}
...
class HorizontalLinePrimitive extends DrawingPrimitiveBase {
  constructor(chart, series, price, color) {
    super(chart, series);
    this.price = price;
    this.color = color || DRAW_COLOR;
  }
...
class VerticalLinePrimitive extends DrawingPrimitiveBase {
  constructor(chart, series, time, color) {
    super(chart, series);
    this.time = time;
    this.color = color || DRAW_COLOR;
  }
...
class DrawingToolManager {
  ...
  _onMove(param) {
    if (this.activeTool === "horizontal" || this.activeTool === "vertical") {
      this.hover = param;
      return;
    }
    this.hover = param;
    ...
    switch (this.activeTool) {
      ...
      case "ray":
        this.preview = new RayPrimitive(this.chart, this.series, first, p);
        break;
  ...
  _commitDrawing(tool, pts) {
    ...
    switch (tool) {
      ...
      case "ray":
        primitive = new RayPrimitive(this.chart, this.series, pts[0], pts[1]);
        break;
...
const TOOL_ICONS = {
  ...
  horizontal: `<line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" stroke-width="1.6" stroke-dasharray="3,2"/>`,
  vertical: `<line x1="12" y1="3" x2="12" y2="21" stroke="currentColor" stroke-width="1.6" stroke-dasharray="3,2"/>`,
